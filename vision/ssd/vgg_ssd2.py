...
from .config import vgg_ssd_config2 as config


def create_vgg_ssd2(num_classes, is_test=False):
    ...
    extras = ModuleList([
        ...
        Sequential(
            Conv2d(in_channels=256, out_channels=128, kernel_size=1),
            ReLU(),
            Conv2d(in_channels=128, out_channels=256, kernel_size=4, stride=1, padding=1),
            ReLU()
        )
    ])
...
def create_vgg_ssd_predictor2(net, candidate_size=200, nms_method=None, iou_threshold=config.iou_threshold, sigma=0.5, device=None):
